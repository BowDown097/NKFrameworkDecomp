...
#include <NewFramework/Math/Maths.h>

#include <NewFramework/Utilities/StringHelper.h>

#include <cmath>

CMatrix::CMatrix(int identity) : mData({}) {
	if (identity > 0) {
		Identity();
	}
}
...
CVec3 CMatrix::UnProject(const CVec2& screenPos, float depth, const CVec2& viewport, const CMatrix& projMatrix) const {
	...
	CMatrix invProjMatrix{1};
	if (!Inverse(invProjMatrix)) {
		return {};
	}

	// Multiply the inverse projection matrix by the NDC vector
	CVec4 clipPos = invProjMatrix * CVec4{ndcPos, 0.0f};
