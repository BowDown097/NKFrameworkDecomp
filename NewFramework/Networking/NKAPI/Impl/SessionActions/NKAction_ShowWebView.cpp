...
void BA_ShowWebView::Start(BehaviourTree::IBlackboard* blackboard)
{
    ...
    boost::shared_ptr<C_NKLoginWebView> webView(Create_NKLoginWebview(
    ...
    this->blackboard->webView = webView;
...
void BA_ShowWebView::Update(BehaviourTree::IBlackboard*, const float& elapsed)
{
    ...
    if (!webViewLoaded)
    {
        float timeoutDiff = timeout - elapsed;
        timeout = timeoutDiff;
        if (timeoutDiff <= 0.0f)
        {
...
{
    BA_ShowWebView* out = new BA_ShowWebView;
    out->state = state;
    out->lastState = state;
